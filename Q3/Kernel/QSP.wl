...
Jacobian[qsp:QSP[phi_?VectorQ -> parity:(-1|1), ___]] := Module[
  ...
  yy = Gradient[qsp, xx];
  Map[theChebyshevFourier[# -> parity]&, yy]
]
...
Options[QSPFind] = {
  Method -> "FindMinimum", (* "L-BFGS", "Newton" *)
...
qspFindByFPI[coeff_?VectorQ -> parity:(-1|1), opts:OptionsPattern[QSPFind]] := Module[
  { itr = 0,
...
qspFindByNewton[coeff_?VectorQ -> parity:(-1|1), opts:OptionsPattern[QSPFind]] := Module[
  { itr = 0,
    max = OptionValue[MaxIterations],
    tol = OptionValue[Tolerance],
    phi = OptionValue["InitialGuess"],
...
qspFindByBFGS[coeff_?VectorQ -> parity:(-1|1), OptionsPattern[{QSPFind, qspFindByBFGS}]] := Module[
  { itr = 0,
    max = OptionValue[MaxIterations],
    tol = OptionValue[Tolerance],
    phi = OptionValue["InitialGuess"],
...
qspFindByFindMinimum[coeff_?VectorQ -> parity:(-1|1), opts:OptionsPattern[{QSPFind, FindMinimum}]] := Module[
  { itr = 0,
...
qspFindByNMinimize[coeff_?VectorQ -> parity:(-1|1), opts:OptionsPattern[{QSPFind, NMinimize}]] := Module[
  { pst = OptionValue["PostProcessor"],
    pnt = ChebyshevPoints[Length[coeff] -> parity],

...
fileDeploy[src_String, dst_String, OptionsPattern[PlaybookDeploy]] := Module[
  ...
  SetOptions[nb, StyleDefinitions -> FrontEnd`FileName[{"Q3"}, "PlaybookNub.nb",CharacterEncoding -> "UTF-8"]];
...
PlaybookCheckIn[nb_NotebookObject] := (
  ...
  SetOptions[nb, StyleDefinitions -> FrontEnd`FileName[{"Q3"}, "Playbook.nb",CharacterEncoding -> "UTF-8"], Saveable -> True];  
)
...
PlaybookCheckOut[nb_NotebookObject] := (
  NotebookDelete[EvaluationCell[]];
  SetOptions[nb, StyleDefinitions -> FrontEnd`FileName[{"Q3"}, "PlaybookNub.nb",CharacterEncoding -> "UTF-8"], Saveable -> False];
  NotebookSave[nb];
)
...
PlaybookBanner[title_String:$PlaybookBannerTitle, opts:OptionsPattern[{PlaybookBanner, Cell}]] := 
  If[ OptionValue["Deployed"],
    ...
    Module[
      { logo, data },
      logo = Thumbnail[
        Import[PacletObject["QuantumMob/Q3"]["AssetLocation", "Q3 Emblem Medium"]],
        Medium
      ];

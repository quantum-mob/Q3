(* -*- mode:math -*- *)
BeginPackage["QuantumMob`PlaybookTools`"]
...
fileDeploy[src_String, dst_String, OptionsPattern[PlaybookDeploy]] := Module[
  ...
  SetOptions[nb, StyleDefinitions -> FrontEnd`FileName[{"Q3S"}, "PlaybookNub.nb",CharacterEncoding -> "UTF-8"]];
...
PlaybookCheckIn[nb_NotebookObject] := (
  ...
  SetOptions[nb, StyleDefinitions -> FrontEnd`FileName[{"Q3S"}, "Playbook.nb",CharacterEncoding -> "UTF-8"], Saveable -> True];  
)
...
PlaybookCheckOut[nb_NotebookObject] := (
  NotebookDelete[EvaluationCell[]];
  SetOptions[nb, StyleDefinitions -> FrontEnd`FileName[{"Q3S"}, "PlaybookNub.nb",CharacterEncoding -> "UTF-8"], Saveable -> False];
  NotebookSave[nb];
)
...
$PlaybookBannerTitle = "Q3S: Symbolic Quantum Simulation";
...
PlaybookBanner[title_String:$PlaybookBannerTitle, opts:OptionsPattern[{PlaybookBanner, Cell}]] := 
  If[ OptionValue["Deployed"],
    ...
    Module[
      { logo, data },
      logo = Thumbnail[
        Import[PacletObject["QuantumMob/Q3S"]["AssetLocation", "Q3 Emblem Medium"]],
        Medium
      ];
